...
export class RideService implements OnModuleInit {
  ...
  private distancePointToSegmentMeters(
    ...
    const t = Math.max(
      0,
      Math.min(1, ((px - ax) * abx + (py - ay) * aby) / (abx * abx + aby * aby || 1e-9)),
    );
    const cx = ax + abx * t,
      cy = ay + aby * t;
  ...
  private distanceToPolylineMeters(
    p: { lat: number; lng: number },
    poly: Array<{ lat: number; lng: number }>,
  ) {
    let min = Number.POSITIVE_INFINITY;
    for (let i = 0; i < poly.length - 1; i++)
      min = Math.min(min, this.distancePointToSegmentMeters(p, poly[i], poly[i + 1]));
  ...
  private async createSafetyAlert(
    ...
    const alert = await this.prisma.safetyAlert.create({
      data: { trip_id: tripId, type, severity, message, payload_json: payload as any },
    });
    await this.addEvent(tripId, actorUserId, 'trip.safety.alert_created', {
      alert_id: alert.id,
      type,
      severity,
    });
    this.ws.emitTrip(tripId, 'safety.alert', { id: alert.id, type, severity, message });
    this.ws.emitSosAlert('sos.alert.created', {
      id: alert.id,
      trip_id: tripId,
      type,
      severity,
    ...
    if (actorUserId) {
      if (type === SafetyAlertType.ENTERED_RED_ZONE) {
        await this.scoreService.applyScoreEvent({
          user_id: actorUserId,
          actor_type: ActorType.DRIVER,
          type: ScoreEventType.ENTERED_RED_ZONE,
          delta: -10,
          trip_id: tripId,
        ...
        await this.scoreService.applyScoreEvent({
          user_id: actorUserId,
          actor_type: ActorType.DRIVER,
          type: ScoreEventType.ROUTE_DEVIATION_MINOR,
          delta: -5,
          trip_id: tripId,
          safety_alert_id: alert.id,
        });
        ...
        await this.scoreService.applyScoreEvent({
          ...
          type: ScoreEventType.OTP_FAILED_MULTIPLE,
          delta: -12,
          trip_id: tripId,
          safety_alert_id: alert.id,
        });
    ...
    if (type === SafetyAlertType.TRACKING_LOST && severity >= 4) {
      ...
      if (trip?.driver_user_id)
        await this.scoreService.applyScoreEvent({
          ...
          actor_type: ActorType.DRIVER,
          type: ScoreEventType.TRACKING_LOST_MAJOR,
          delta: -10,
          trip_id: tripId,
          safety_alert_id: alert.id,
        });
  ...
  private async applySafetyScore(tripId: string, delta: number) {
    const state = await this.prisma.tripSafetyState.upsert({
      where: { trip_id: tripId },
      update: { safety_score: { decrement: Math.abs(delta) } },
      create: { trip_id: tripId, safety_score: 100 - Math.abs(delta) },
    });
    ...
    if (score <= 70)
      this.ws.emitTrip(tripId, 'safety.checkin_required', { trip_id: tripId, safety_score: score });
    if (score <= 50)
      await this.createSafetyAlert(
        tripId,
        SafetyAlertType.MANUAL_SOS_TRIGGER,
        4,
        'Safety score critically low: contact SOS suggested',
        { safety_score: score },
        null,
      );
    if (score <= 35)
      await this.addEvent(tripId, null, 'trip.safety.flagged', { safety_score: score });
  }
  ...
  async requestTrip(
    ...
    const passengerScore = await this.scoreService.getOrCreateUserScore(
      passengerUserId,
      ActorType.PASSENGER,
    );
    await this.fraud.captureFingerprint(passengerUserId, ActorType.PASSENGER, headers ?? {});
    const accountHold = await this.prisma.userHold.findFirst({
      where: {
        user_id: passengerUserId,
        hold_type: HoldType.ACCOUNT_BLOCK,
    ...
    const peakGate = await this.merit.evaluatePeakGate(
      passengerUserId,
      ActorType.PASSENGER,
      passengerScore.score,
      passengerScore.status,
    );
    ...
    const premiumCfg = ((
      await this.prisma.appConfig.findUnique({ where: { key: 'premium_zones' } })
    )?.value_json as any) ?? { deny_low_passenger: false };
    if (premium.zone && !premium.eligible && premiumCfg.deny_low_passenger)
      throw new ForbiddenException('Passenger score not eligible for premium zone');

    const isRestrictedPassenger =
    ...
    await this.prisma.tripRouteBaseline.create({
      data: { trip_id: trip.id, polyline_json: baseline as any },
    });
    await this.prisma.tripSafetyState.create({
      data: { trip_id: trip.id, safety_score: 100, deviation_level: DeviationLevel.NONE },
    });

    await this.addEvent(trip.id, passengerUserId, 'trip.created', {
      price_base,
      baseline: 'heuristic_mvp',
    });
    await this.addEvent(trip.id, passengerUserId, 'trip.bidding.started', {
      bidding_expires_at: expires.toISOString(),
      merit_mode: isRestrictedPassenger ? 'limited' : 'normal',
    });

    this.ws.emitTrip(trip.id, 'trip.created', { trip_id: trip.id, status: trip.status });
    this.ws.emitToUser(passengerUserId, 'trip.priority.info', {
      trip_id: trip.id,
      mode: isRestrictedPassenger ? 'limited' : 'normal',
    ...
    const maxDistance = Math.max(
      ...activeNearby.map((p) =>
        this.haversineKm(
          dto.origin_lat,
          dto.origin_lng,
          p.last_lat ?? dto.origin_lat,
          p.last_lng ?? dto.origin_lng,
        ),
      ),
      1,
    );
    ...
    for (const p of activeNearby) {
      ...
      const statusBonus =
        us?.status === RestrictionStatus.NONE
          ? 1
          : us?.status === RestrictionStatus.WARNING
            ? 0.5
            : 0.2;
  ...
  async createBid(
    ...
    if (!presence || !presence.is_online || !this.onlineRecent(presence.last_seen_at))
      throw new ForbiddenException('Driver is offline');

    await this.fraud.captureFingerprint(driverUserId, ActorType.DRIVER, headers ?? {});

    const mp = await this.prisma.externalDriverProfile.findUnique({
      where: { user_id: driverUserId },
    });
    if (!mp?.mp_account_id)
      throw new ForbiddenException(
        'Driver must connect MercadoPago account before accepting trips',
      );

    const min = Math.round(trip.price_base * 0.7),
      max = Math.round(trip.price_base * 2.0);
    ...
    this.ws.emitTrip(trip.id, 'trip.bid.received', {
      bid_id: bid.id,
      driver_user_id: driverUserId,
      price_offer: bid.price_offer,
    });
  ...
  async acceptBid(tripId: string, passengerUserId: string, dto: AcceptBidDto) {
    ...
    this.ws.emitToDriver(bid.driver_user_id, 'trip.matched', {
      trip_id: tripId,
      passenger_user_id: passengerUserId,
    });
    return updated;
  ...
  async autoMatchExpiredBiddingTrips() {
    ...
    const trips = await this.prisma.trip.findMany({
      where: { status: TripStatus.BIDDING, bidding_expires_at: { lt: now } },
    });
    for (const trip of trips) {
      const bids = await this.prisma.tripBid.findMany({
        where: { trip_id: trip.id, status: TripBidStatus.PENDING },
      });
      if (!bids.length) {
        const updated = await this.prisma.trip.updateMany({
          where: { id: trip.id, status: TripStatus.BIDDING },
          data: { status: TripStatus.EXPIRED_NO_DRIVER },
        });
        if (updated.count === 0) continue;
        await this.addEvent(trip.id, null, 'trip.expired_no_driver', {});
        this.ws.emitTrip(trip.id, 'trip.cancelled', {
          trip_id: trip.id,
          status: TripStatus.EXPIRED_NO_DRIVER,
        });
        continue;
      }
      ...
      const tx = await this.prisma.$transaction(async (trx) => {
        ...
        await trx.tripBid.updateMany({
          where: { trip_id: trip.id, id: { not: best.id }, status: TripBidStatus.PENDING },
          data: { status: TripBidStatus.REJECTED },
        });
        await trx.tripBid.update({
          where: { id: best.id },
          data: { status: TripBidStatus.AUTO_SELECTED },
        });
      ...
      this.ws.emitTrip(trip.id, 'trip.matched', {
        trip_id: trip.id,
        driver_user_id: best.driver_user_id,
        auto_selected: true,
      });
  ...
  async driverEnRoute(tripId: string, driverUserId: string) {
    ...
    const updated = await this.prisma.trip.update({
      where: { id: tripId },
      data: { status: TripStatus.DRIVER_EN_ROUTE },
    });
  ...
  async driverArrived(tripId: string, driverUserId: string) {
    ...
    await this.prisma.trip.update({
      where: { id: tripId },
      data: { status: TripStatus.OTP_PENDING },
    });
    ...
    await this.prisma.tripOTP.upsert({
      ...
      update: {
        ...
        expires_at: new Date(this.nowMs() + 10 * 60 * 1000),
        attempts: 0,
        verified_at: null,
      },
      create: {
  ...
  async verifyOtp(tripId: string, driverUserId: string, dto: VerifyOtpDto) {
    ...
    if (this.otpHash(dto.otp) !== otp.otp_hash) {
      await this.prisma.tripOTP.update({
        where: { trip_id: tripId },
      ...
      if (otp.attempts + 1 >= 3)
        await this.createSafetyAlert(
          tripId,
          SafetyAlertType.OTP_FAILED_MULTIPLE,
          3,
    ...
    const updated = await this.prisma.trip.update({
      where: { id: tripId },
      data: { status: TripStatus.IN_PROGRESS, started_at: new Date() },
    });
  ...
  async trackLocation(tripId: string, driverUserId: string, dto: LocationDto) {
    ...
    await this.prisma.tripSafetyState.upsert({
      where: { trip_id: tripId },
      update: { last_driver_location_at: new Date() },
    ...
    for (const z of zones.sort((a, b) =>
      ...
      if (Array.isArray(poly) && this.pointInPolygon({ lat: dto.lat, lng: dto.lng }, poly)) {
        zoneType = z.type;
        break;
      }
    }
    ...
    if (zoneType && ss?.last_zone_type !== zoneType) {
      const type =
        zoneType === GeoZoneType.RED
          ? SafetyAlertType.ENTERED_RED_ZONE
          : SafetyAlertType.ENTERED_CAUTION_ZONE;
      const sev = zoneType === GeoZoneType.RED ? 5 : 3;
      await this.createSafetyAlert(
        tripId,
        type,
        sev,
        `Entered ${zoneType} zone`,
        { lat: dto.lat, lng: dto.lng },
        driverUserId,
      );
      await this.prisma.tripSafetyState.update({
        where: { trip_id: tripId },
        data: { last_zone_type: zoneType },
      });
    ...
    if (Array.isArray(line) && line.length > 1) {
      ...
      if (over700For20s) {
        ...
        await this.prisma.tripSafetyState.update({
          where: { trip_id: tripId },
          data: { deviation_level: DeviationLevel.MAJOR },
        });
        await this.createSafetyAlert(
          tripId,
          SafetyAlertType.ROUTE_DEVIATION_MAJOR,
          4,
          'Major route deviation detected',
          { deviation_m: deviationM, sustained_s: 20, major_count: window.majorCount },
          driverUserId,
        );
        ...
        if (window.majorCount >= 2) {
          await this.createSafetyAlert(
            ...
            SafetyAlertType.ROUTE_DEVIATION_MAJOR,
            5,
            'Repeated major deviations detected',
            { major_count: window.majorCount },
            driverUserId,
          );
        }
        ...
        await this.prisma.tripSafetyState.update({
          where: { trip_id: tripId },
          data: { deviation_level: DeviationLevel.MINOR },
        });
        await this.createSafetyAlert(
          tripId,
          SafetyAlertType.ROUTE_DEVIATION_MINOR,
          2,
          'Minor route deviation detected',
          { deviation_m: deviationM, sustained_s: 20 },
          driverUserId,
        );
  ...
  async scanTrackingLostTrips() {
    const trips = await this.prisma.trip.findMany({
      where: { status: { in: [TripStatus.DRIVER_EN_ROUTE, TripStatus.IN_PROGRESS] } },
      include: { safety_state: true },
    });
    const now = this.nowMs();
  ...
  async completeTrip(tripId: string, driverUserId: string) {
    ...
    const updated = await this.prisma.trip.update({
      where: { id: tripId },
      data: { status: TripStatus.COMPLETED, completed_at: new Date() },
    });
    ...
    await this.scoreService.applyRecoveryOnTripCompletion(
    ...
    const thresholds =
      ((await this.prisma.appConfig.findUnique({ where: { key: 'fraud_thresholds' } }))
        ?.value_json as any) ?? {};

    const pairTrips24 = await this.prisma.trip.findMany({
      where: {
        passenger_user_id: trip.passenger_user_id,
        driver_user_id: driverUserId,
    ...
    const pairTrips7 = await this.prisma.trip.count({
      where: {
        passenger_user_id: trip.passenger_user_id,
        driver_user_id: driverUserId,
        status: TripStatus.COMPLETED,
        completed_at: { gte: since7d },
      },
    });
    ...
    if (
      pair24 >= (thresholds.repeated_pair_min_trips_for_pattern ?? 6) &&
      (pair24 > (thresholds.repeated_pair_24h ?? 4) || pair7 > (thresholds.repeated_pair_7d ?? 12))
    ) {
      const lowDistanceKm = thresholds.repeated_pair_low_distance_km ?? 2.0;
      const lowDistanceCount = pairTrips24.filter(
        (t) => (t.distance_km ?? 999) < lowDistanceKm,
      ).length;
      ...
      const sameOrigin = originCenter
        ? pairTrips24.filter(
            (t) =>
              this.haversineKm(
                t.origin_lat,
                t.origin_lng,
                originCenter.origin_lat,
                originCenter.origin_lng,
              ) *
                1000 <=
              (thresholds.repeated_pair_same_origin_radius_m ?? 250),
          ).length
        : 0;
      const sameDest = destCenter
        ? pairTrips24.filter(
            (t) =>
              this.haversineKm(t.dest_lat, t.dest_lng, destCenter.dest_lat, destCenter.dest_lng) *
                1000 <=
              (thresholds.repeated_pair_same_dest_radius_m ?? 250),
          ).length
        : 0;
      ...
      const highPattern =
        (thresholds.repeated_pair_requires_low_distance ?? true)
      ...
      const payload = {
        pair_key: `${driverUserId}:${trip.passenger_user_id}`,
        count_24h: pair24,
        count_7d: pair7,
        low_distance_ratio: lowDistanceRatio,
        origin_cluster: originSimilarity,
        dest_cluster: destSimilarity,
        amount_tight: amountTight,
      };
      await this.fraud.applySignal({
        ...
        payload: { ...payload, driver_user_id: driverUserId },
      });
      await this.fraud.applySignal({
        user_id: driverUserId,
        trip_id: tripId,
        ...
        severity,
        score_delta: scoreDelta,
        payload: { ...payload, passenger_user_id: trip.passenger_user_id },
      });
    }
  ...
  async rateTrip(tripId: string, passengerUserId: string, dto: RateTripDto) {
    ...
    if (trip.status !== TripStatus.COMPLETED)
      throw new BadRequestException('Trip is not completed');
    await this.addEvent(tripId, passengerUserId, 'trip.rated', {
      rating: dto.rating,
  ...
  async cancelPassenger(tripId: string, passengerUserId: string, dto: CancelDto) {
    ...
    if (trip.passenger_user_id !== passengerUserId)
      throw new ForbiddenException('Not trip passenger');
    if (trip.status === TripStatus.IN_PROGRESS && dto.reason !== CancelReason.SAFETY)
      throw new BadRequestException('Cannot cancel in_progress without SAFETY');
    if (
      trip.status === TripStatus.MATCHED &&
      trip.driver_user_id &&
      dto.reason !== CancelReason.SAFETY
    ) {
    ...
    const updated = await this.prisma.trip.update({
      ...
      data: {
        ...
        cancelled_by_user_id: passengerUserId,
    ...
    const penalty =
      trip.status === TripStatus.DRIVER_EN_ROUTE
        ? 'light'
        : trip.status === TripStatus.MATCHED && trip.driver_user_id
          ? 'moderate'
          : 'none';
    await this.addEvent(tripId, passengerUserId, 'trip.cancelled', {
      by: 'passenger',
      reason: dto.reason,
      penalty,
    });
  ...
  async cancelDriver(tripId: string, driverUserId: string, dto: CancelDto) {
    ...
    const updated = await this.prisma.trip.update({
      ...
      data: {
        status,
        cancelled_at: new Date(),
        cancelled_by_user_id: driverUserId,
        cancel_reason: dto.reason,
      },
    });
    await this.addEvent(tripId, driverUserId, 'trip.cancelled', {
      by: 'driver',
      reason: dto.reason,
      penalty: trip.status === TripStatus.DRIVER_EN_ROUTE ? 'strong' : 'none',
    });
    if (trip.status === TripStatus.DRIVER_EN_ROUTE) {
      await this.scoreService.applyScoreEvent({
        user_id: driverUserId,
  ...
  async tripDetail(id: string) {
    const trip = await this.prisma.trip.findUnique({
      ...
      include: {
        ...
        locations: { orderBy: { created_at: 'desc' }, take: 300 },
        bids: true,
        safety_alerts: { orderBy: { created_at: 'desc' } },
        safety_state: true,
      },
    });
    if (!trip) throw new NotFoundException('Trip not found');
