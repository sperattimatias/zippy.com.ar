...
export class RideService implements OnModuleInit {
  ...
  async driverArrived(tripId: string, driverUserId: string) {
    ...
    await this.prisma.tripOTP.upsert({ where: { trip_id: tripId }, update: { otp_hash: this.otpHash(otp), expires_at: new Date(this.nowMs() + 10 * 60 * 1000), attempts: 0, verified_at: null }, create: { trip_id: tripId, otp_hash: this.otpHash(otp), expires_at: new Date(this.nowMs() + 10 * 60 * 1000), attempts: 0 } });
  ...
  async trackLocation(tripId: string, driverUserId: string, dto: LocationDto) {
    ...
    if (!allowedStatuses.includes(trip.status)) throw new BadRequestException('Invalid status for location');
