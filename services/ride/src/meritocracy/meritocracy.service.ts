...
export class MeritocracyService {
  ...
  computeBadgeTier(
    score: number,
    thresholds?: { excellent: number; trusted: number; watchlist: number },
  ) {
  ...
  async updateBadge(userId: string, actorType: ActorType, score: number) {
    const cfg = await this.getConfig('score_thresholds', {
      badge: { excellent: 90, trusted: 75, watchlist: 60 },
    });
    ...
    this.ws.emitToUser(userId, 'user.badge.updated', {
  ...
  async evaluatePeakGate(
    userId: string,
    actorType: ActorType,
    score: number,
    status: RestrictionStatus,
  ) {
    const cfg = await this.getConfig('peak_hours', {
      driver_min_score: 50,
    ...
    if (!peak)
      return {
        allowed: true,
        limitedMode: status === RestrictionStatus.LIMITED,
        reason: 'off_peak',
      };
    if (status === RestrictionStatus.BLOCKED) {
      await this.prisma.peakGateEvent.create({
        data: { user_id: userId, actor_type: actorType, allowed: false, reason: 'blocked_status' },
      });
      this.ws.emitToUser(userId, 'peak.gate.denied', {
        actor_type: actorType,
    ...
    const min =
      actorType === ActorType.DRIVER
        ? (cfg.driver_min_score ?? 50)
        : (cfg.passenger_min_score ?? 45);
    if (score < min) {
      await this.prisma.peakGateEvent.create({
        data: {
          user_id: userId,
  ...
  async getPremiumContext(
    ...
    const zone = zones.find(
      (z) => Array.isArray(z.polygon_json) && this.pointInPolygon(point, z.polygon_json as any),
    );
    if (!zone) return { zone: null, eligible: true, premium_bonus: 0 };
    const eligible =
      actorType === ActorType.DRIVER
        ? actorScore >= zone.min_driver_score
        : actorScore >= zone.min_passenger_score;
