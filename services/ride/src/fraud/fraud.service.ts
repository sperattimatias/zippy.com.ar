...
export class FraudService {
  ...
  private severityOrder(s: FraudSeverity) {
    return s === FraudSeverity.CRITICAL
      ? 4
      : s === FraudSeverity.HIGH
        ? 3
        : s === FraudSeverity.MEDIUM
          ? 2
          : 1;
  }
  ...
  private async getLatestSimilarSignal(input: {
    user_id?: string;
  ...
  async applySignal(input: {
    ...
    const existing = await this.getLatestSimilarSignal({
    ...
    if (existing) {
      ...
      await this.prisma.fraudSignal.update({
        ...
        data: { payload_json: merged as any },
      });
      return {
        signal: { ...existing, payload_json: merged },
        fraud_case: await this.createOrAttachCase({ ...existing, payload_json: merged }),
      };
    }
    ...
    if (input.user_id) {
      const current = await this.prisma.financialRiskScore.upsert({
        where: { user_id: input.user_id },
        update: {},
        create: { user_id: input.user_id },
      });
      const nextScore = Math.max(0, Math.min(100, current.score + input.score_delta));
      const nextLevel = this.riskLevel(nextScore);
      await this.prisma.financialRiskScore.update({
        where: { user_id: input.user_id },
        data: { score: nextScore, level: nextLevel },
      });

      if (nextLevel === 'HIGH')
        await this.createHoldIfAbsent(
          input.user_id,
          HoldType.FEATURE_LIMIT,
          'Auto hold by high financial risk',
          48,
        );
      if (nextLevel === 'CRITICAL')
        await this.createHoldIfAbsent(
          input.user_id,
          HoldType.PAYOUT_HOLD,
          'Auto payout hold by critical financial risk',
        );
    }

    const fraudCase = await this.createOrAttachCase(signal);
    this.ws.emitSosAlert('admin.fraud.case.created', {
  ...
  private async createOrAttachCase(signal: any) {
    ...
    const existing = pairKey
      ? (candidates.find((c) => ((c.related_user_ids as any)?.pair_key ?? null) === pairKey) ??
        candidates[0])
      : candidates[0];
    ...
    const fraudCase =
      ...
      (await this.prisma.fraudCase.create({
        data: {
          ...
          related_user_ids: signal.user_id
            ? ({ users: [signal.user_id], ...(pairKey ? { pair_key: pairKey } : {}) } as any)
            : null,
    ...
    if (existing) {
      const sev =
        this.severityOrder(signal.severity) > this.severityOrder(existing.severity)
          ? signal.severity
          : existing.severity;
      await this.prisma.fraudCase.update({
        where: { id: existing.id },
        data: { severity: sev, last_signal_at: signal.created_at },
      });
    }

    await this.prisma.fraudCaseSignalLink.upsert({
      where: {
        fraud_case_id_fraud_signal_id: { fraud_case_id: fraudCase.id, fraud_signal_id: signal.id },
      },
      update: {},
  ...
  async createHoldIfAbsent(
    ...
    const now = new Date();
    const active = await this.prisma.userHold.findFirst({
      where: {
        user_id: userId,
        hold_type: holdType,
  ...
  async releaseHold(id: string, byUserId?: string) {
    const hold = await this.prisma.userHold.update({
      where: { id },
      data: {
        status: HoldStatus.RELEASED,
        ends_at: new Date(),
        created_by_user_id: byUserId ?? undefined,
      },
    });
  ...
  async listCases(filter: { status?: FraudCaseStatus; severity?: FraudSeverity; q?: string }) {
    return this.prisma.fraudCase.findMany({
      where: {
        ...
        ...(filter.q
          ? { OR: [{ title: { contains: filter.q } }, { summary: { contains: filter.q } }] }
          : {}),
      },
      orderBy: { created_at: 'desc' },
      take: 300,
  ...
  async getCase(id: string) {
    ...
    const links = await this.prisma.fraudCaseSignalLink.findMany({
      where: { fraud_case_id: id },
      orderBy: { created_at: 'desc' },
    });
    const signals = await this.prisma.fraudSignal.findMany({
      where: { id: { in: links.map((l) => l.fraud_signal_id) } },
    });
  ...
  async assignCase(id: string, assignedToUserId: string) {
    return this.prisma.fraudCase.update({
      where: { id },
      data: { status: FraudCaseStatus.IN_REVIEW, assigned_to_user_id: assignedToUserId },
    });
  }
  async resolveCase(id: string, notes: string) {
    return this.prisma.fraudCase.update({
      where: { id },
      data: { status: FraudCaseStatus.RESOLVED, summary: notes || 'resolved' },
    });
  }
  async dismissCase(id: string, notes: string) {
    return this.prisma.fraudCase.update({
      where: { id },
      data: { status: FraudCaseStatus.DISMISSED, summary: notes || 'dismissed' },
    });
  }
  ...
  async autoCloseLowCases() {
    ...
    const cases = await this.prisma.fraudCase.findMany({
      where: {
        ...
        OR: [{ last_signal_at: null }, { last_signal_at: { lt: threshold } }],
      },
      take: 500,
    });
    for (const c of cases)
      await this.prisma.fraudCase.update({
        where: { id: c.id },
        data: { status: FraudCaseStatus.RESOLVED, summary: 'Auto-closed: no new signals' },
      });
  }
  ...
  async decayRiskScores() {
    ...
    for (const rs of scores) {
      const hasSignals = await this.prisma.fraudSignal.count({
        where: { user_id: rs.user_id, created_at: { gte: since30 } },
      });
      if (hasSignals > 0 || rs.score <= 0) continue;
      const nextScore = Math.max(0, rs.score - decay);
      await this.prisma.financialRiskScore.update({
        where: { user_id: rs.user_id },
        data: { score: nextScore, level: this.riskLevel(nextScore) },
      });
      await this.prisma.fraudSignal.create({
        data: {
          user_id: rs.user_id,
          type: FraudSignalType.RISK_DECAY,
          severity: FraudSeverity.LOW,
          score_delta: -decay,
          payload_json: { reason: 'no_signals_30d' } as any,
        },
      });
    }
  }
  ...
  async runPeriodicDetections() {
    const thresholds = ((
      ...
      shared_ip_users_24h: 6,
      shared_device_users_24h: 3,
      shared_ip_severity_max: 'LOW',
    };
    const since = new Date(Date.now() - 24 * 3600 * 1000);
    const fps = await this.prisma.clientFingerprint.findMany({
    ...
    for (const fp of fps) {
      ...
      if (fp.device_fingerprint_hash) {
        if (!byDevice.has(fp.device_fingerprint_hash))
          byDevice.set(fp.device_fingerprint_hash, new Set());
    ...
    for (const [ip, users] of byIp) {
      if (users.size > (thresholds.shared_ip_users_24h ?? 6)) {
        const hasDeviceOverlap = [...users].some((u) =>
          [...byDevice.values()].some(
            (set) => set.has(u) && set.size > (thresholds.shared_device_users_24h ?? 3),
          ),
        );
        const severity = hasDeviceOverlap
          ? FraudSeverity.MEDIUM
          : (FraudSeverity[
              (thresholds.shared_ip_severity_max ?? 'LOW') as keyof typeof FraudSeverity
            ] ?? FraudSeverity.LOW);
        ...
        for (const user of users)
          await this.applySignal({
            ...
            severity,
            score_delta: delta,
            payload: { users: users.size, ip_hash: ip },
          });
      }
    }
    ...
    for (const [, users] of byDevice) {
      if (users.size > (thresholds.shared_device_users_24h ?? 3)) {
        for (const user of users) {
          await this.applySignal({
            user_id: user,
            type: FraudSignalType.SHARED_DEVICE_FINGERPRINT,
            severity: users.size > 5 ? FraudSeverity.CRITICAL : FraudSeverity.HIGH,
            score_delta: users.size > 5 ? 30 : 20,
            payload: { users: users.size },
          });
        }
      }
    }
