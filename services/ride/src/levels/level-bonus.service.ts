...
export class LevelAndBonusService {
  ...
  private levelFromRules(
    ...
    const ok = {
      ...
      diamond:
        ...
        perf.tripsCompleted >=
          (rules?.diamond?.trips_completed_last30_gte ?? (passenger ? 80 : 150)) &&
  ...
  async computeDriverLevel(userId: string, now = new Date()) {
    ...
    const activeHold = await this.prisma.userHold.findFirst({
      where: {
        user_id: userId,
        status: 'ACTIVE',
        hold_type: { in: ['PAYOUT_HOLD', 'ACCOUNT_BLOCK'] },
        OR: [{ ends_at: null }, { ends_at: { gt: now } }],
      },
    });
    const premiumTiers: LevelTier[] = [LevelTier.GOLD, LevelTier.DIAMOND];
    if (activeHold && premiumTiers.includes(tier)) {
  ...
  async computeMonthlyPerformance(year: number, month: number) {
    ...
    const completedIds = [
      ...new Set(driverTripsRaw.filter((t) => t.status === TripStatus.COMPLETED).map((t) => t.id)),
    ];
    const refundedFull = completedIds.length
      ...
      : [];
    const fullyRefundedTripIds = new Set(
      refundedFull.filter((p) => p.refunded_amount >= p.amount_total).map((p) => p.trip_id),
    );
    const driverTrips = driverTripsRaw.filter(
      (t) => !(t.status === TripStatus.COMPLETED && fullyRefundedTripIds.has(t.id)),
    );
    const passengerTrips = passengerTripsRaw.filter(
      (t) => !(t.status === TripStatus.COMPLETED && fullyRefundedTripIds.has(t.id)),
    );
    ...
    const collect = async (
      ...
      for (const userId of users) {
            ...
            where: {
              ...
              type:
                actor === ActorType.DRIVER
                  ? ScoreEventType.DRIVER_NO_SHOW
                  : ScoreEventType.PASSENGER_NO_SHOW,
        ...
        await this.prisma.monthlyPerformance.upsert({
          where: {
            user_id_actor_type_year_month: { user_id: userId, actor_type: actor, year, month },
          },
          update: {
    ...
    const dUsers = [
      ...new Set(driverTrips.map((t) => t.driver_user_id).filter(Boolean) as string[]),
    ];
    const pUsers = [...new Set(passengerTrips.map((t) => t.passenger_user_id))];
    await collect(ActorType.DRIVER, dUsers, (u) => ({
      completed: driverTrips.filter(
        (t) => t.driver_user_id === u && t.status === TripStatus.COMPLETED,
      ).length,
      cancelled: driverTrips.filter(
        (t) => t.driver_user_id === u && t.status === TripStatus.CANCELLED_BY_DRIVER,
      ).length,
    }));
    await collect(ActorType.PASSENGER, pUsers, (u) => ({
      completed: passengerTrips.filter(
        (t) => t.passenger_user_id === u && t.status === TripStatus.COMPLETED,
      ).length,
      cancelled: passengerTrips.filter(
        (t) => t.passenger_user_id === u && t.status === TripStatus.CANCELLED_BY_PASSENGER,
      ).length,
    }));
  ...
  async computeMonthlyBonuses(year: number, month: number) {
    ...
    for (const r of rows) {
      const baseEligible =
        r.trips_completed >= (rules.min_trips_completed ?? 40) &&
        r.no_show_count === (rules.require_no_show_eq ?? 0) &&
        r.safety_major_alerts === (rules.require_safety_major_alerts_eq ?? 0);
      ...
      const severeSignal = await this.prisma.fraudSignal.count({
        where: {
          user_id: r.user_id,
      ...
      if (hold || severeSignal > 0) {
        this.ws.emitSosAlert('admin.fraud.bonus_denied', {
          ...
          year,
          month,
          reason: hold ? 'active_hold' : 'high_signal',
        });
        continue;
      }
      eligible.push(r);
    ...
    for (let i = 0; i < eligible.length; i++) {
      ...
      const discount =
        percentile <= 0.01
          ? (rules.top_1_discount_bps ?? 800)
          : percentile <= 0.03
            ? (rules.top_3_discount_bps ?? 500)
            : percentile <= 0.1
              ? (rules.top_10_discount_bps ?? 300)
              : 0;
      ...
      const bonus = await this.prisma.monthlyBonusLedger.upsert({
        ...
        update: {
          ...
          reason: `percentile_${Math.round(percentile * 10000) / 100}`,
          starts_at: nextMonthStart,
          ends_at: nextMonthEnd,
        },
        create: {
          driver_user_id: r.user_id,
          year,
          month,
          discount_bps: discount,
          bonus_type: BonusType.COMMISSION_DISCOUNT,
      ...
      this.ws.emitToUser(r.user_id, 'driver.bonus.assigned', {
        year,
        month,
  ...
  async getActiveCommissionBps(driverUserId: string, at = new Date()) {
    ...
    return {
      ...
      bonus_valid_until: bonus?.ends_at ?? null,
    };
  }

  @Cron('0 3 * * *')
  async dailyRecomputeLevels() {
    const activeDrivers = await this.prisma.trip.findMany({
      where: {
    ...
    const activePassengers = await this.prisma.trip.findMany({
      where: { created_at: { gte: new Date(Date.now() - 30 * 24 * 3600 * 1000) } },
      select: { passenger_user_id: true },
    });
    for (const userId of [...new Set(activeDrivers.map((x) => x.driver_user_id!).filter(Boolean))])
      await this.computeDriverLevel(userId);
    for (const userId of [
      ...new Set(activePassengers.map((x) => x.passenger_user_id).filter(Boolean)),
    ])
      await this.computePassengerLevel(userId);
  }
  ...
  async listLevels(actorType?: ActorType, tier?: LevelTier) {
    return this.prisma.userLevel.findMany({
      where: { ...(actorType ? { actor_type: actorType } : {}), ...(tier ? { tier } : {}) },
  ...
  async listMonthlyPerformance(year: number, month: number, actorType?: ActorType) {
    return this.prisma.monthlyPerformance.findMany({
      where: { year, month, ...(actorType ? { actor_type: actorType } : {}) },
      orderBy: { performance_index: 'desc' },
      take: 500,
    });
  }
  ...
  async revokeBonus(id: string, reason: string) {
    return this.prisma.monthlyBonusLedger.update({
      where: { id },
      data: { status: BonusStatus.REVOKED, reason },
    });
  }
}
