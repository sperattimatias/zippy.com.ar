...
describe('PaymentsService', () => {
  it('create preference calculates commission and floor', async () => {
    const prisma: any = {
      ...
      monthlyBonusLedger: {
        findFirst: jest.fn().mockResolvedValue({ discount_bps: 900 }),
      },
  ...
  it('rejected webhook marks settlement failed', async () => {
    ...
    expect(prisma.tripPayment.update).toHaveBeenCalledWith(
      expect.objectContaining({
        data: expect.objectContaining({ settlement_status: SettlementStatus.FAILED }),
      }),
    );
  });

  it('rounding deterministic split always sums refund amount', () => {
    const svc: any = new PaymentsService({} as any);
  ...
  it('full refund flips status to REFUNDED and settlement FAILED', async () => {
    ...
    const prisma: any = {
      ...
      driverPayoutSummary: {
        findUnique: jest.fn().mockResolvedValue({
          total_gross: BigInt(10000),
          total_commission: BigInt(1000),
          total_bonus_discount: BigInt(0),
          total_net: BigInt(9000),
        }),
        upsert: jest.fn(),
      },
      trip: {
    ...
    const svc: any = new PaymentsService(prisma as any);
