  ...
  return (
    <main className="mx-auto max-w-6xl space-y-6 p-6">
      <section className="rounded-xl border border-slate-800 p-4">
        <h1 className="mb-4 text-2xl font-bold">Alertas de seguridad</h1>
        ...
        {rows.map((a) => (
          <article key={a.id} className="rounded-xl border border-slate-800 p-4">
            ...
            <div className="mt-3 space-x-3 text-sm">
              <button className="text-cyan-400" onClick={() => update(a.id, 'ACKNOWLEDGED')}>Reconocer</button>
              <button className="text-emerald-400" onClick={() => update(a.id, 'RESOLVED')}>Resolver</button>
              <button className="text-rose-400" onClick={() => update(a.id, 'DISMISSED')}>Descartar</button>
            </div>
          </article>
        ))}
      </section>
