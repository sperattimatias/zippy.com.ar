...
export async function GET(req: Request) {
  ...
  const upstream = await fetch(`${base}/api/admin/notifications/templates?${url.searchParams.toString()}`, {
...
export async function POST(req: Request) {
  const access = cookies().get('zippy_access_token')?.value;
  if (!access) return new Response('Unauthorized', { status: 401 });
  const upstream = await fetch(`${base}/api/admin/notifications/templates`, {
    method: 'POST',
